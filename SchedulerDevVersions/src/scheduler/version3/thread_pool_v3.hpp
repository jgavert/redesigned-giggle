#pragma once
#include "utils/cpu_info_v3.hpp"
#include "utils/lockfree_queue_v3.hpp"
#include "utils/dynamic_allocator_v3.hpp"
#include <atomic>
#include <vector>
#include <deque>
#include <mutex>
...
class ThreadPool
{
  ...
  struct StealableQueue
  {
    ...
    taskstealer_v3::parallel::per_thread_queue<FreeLoot, 262144> loot; // get it if you can :smirk:
#else
...
public:
  ...
  std::optional<FreeLoot> stealTaskInternal(std::vector<taskstealer_v3::ThreadPool::StealableQueue>& stealQueues, const ThreadData& thread) noexcept {
    FreeLoot stealed = {};
  ...
  bool unforkInternal(std::vector<taskstealer_v3::ThreadPool::StealableQueue>& stealQueues, ThreadData& thread, FreeLoot& loot) noexcept {
  ...
  void spawnTask(std::coroutine_handle<> handle, std::atomic_int* counter, Priority priority) noexcept {
    ...
    {
      taskstealer_v3::ThreadPool::StealableQueue* stealQueue = nullptr;
